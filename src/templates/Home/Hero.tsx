...
export function Hero({ locations }: HeroProps) {
	...
	function buildJobsUrl(search: string, location: string) {
	...
	function handleSearch() {
		router.push(buildJobsUrl(jobTitle, selectedLocation));
	}
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						{popular.map(tag => (
							<button
								...
								className="text-muted-foreground cursor-pointer text-sm font-normal underline-offset-2 hover:underline"
							>
								{tag}
							</button>
						))}
