			...
			<section className="relative overflow-hidden bg-[#F8F8FD] md:bg-[url('/images/hero-pattern.webp')] md:bg-contain md:bg-bottom-right md:bg-no-repeat">
				...
				<div className="relative mx-auto flex max-w-7xl flex-col items-center gap-8 px-6 py-10 md:flex-row md:gap-12 md:py-20">
					...
					<m.div
						...
						animate="visible"
					>
						<m.div variants={fadeInUp} className="flex max-w-125 flex-col gap-2">
						...
						<m.p
							variants={fadeInUp}
							className="text-muted-foreground max-w-md text-base font-normal"
						>
						...
						{/* Search bar */}
						<m.div variants={fadeInUp} className="flex flex-col gap-3 bg-white p-4 shadow-sm">
							<form action="/jobs" className="flex flex-col items-stretch gap-3 md:flex-row">
								{/* Hidden location — submitted as ?location=... */}
								{selectedLocation && (
								...
								<div className="flex flex-1 items-center gap-4">
									...
									<input
										...
										className="h-12 border-0 border-b border-gray-200 text-base font-normal outline-none focus-visible:ring-0 focus-visible:ring-indigo-500"
									/>
								</div>
									...
									<Popover open={open} onOpenChange={setOpen}>
										<PopoverTrigger asChild>
											...
											>
												...
												<div className="flex h-12 flex-1 items-center justify-between border-0 border-b border-gray-200 text-base font-normal text-gray-500">
													<span className="truncate">{selectedLocation || "All Locations"}</span>
													<ChevronDown
														className={`size-4 shrink-0 text-gray-400 transition-transform duration-200 ${open ? "rotate-180" : "rotate-0"}`}
													/>
												</div>
											</div>
										</PopoverTrigger>
												...
												<CommandList>
													...
													<CommandGroup>
														...
														{locations.map(loc => (
															...
															>
																<Check
																	className={`mr-2 h-4 w-4 ${selectedLocation === loc ? "opacity-100" : "opacity-0"}`}
																/>
																{loc}
															</CommandItem>
														))}
						...
						<m.div variants={fadeInUp} className="flex flex-wrap items-center gap-2">
							<span className="text-foreground text-sm font-semibold">Popular:</span>
							{popular.map(tag => (
								<Link
									key={tag}
									href={`/jobs?search=${encodeURIComponent(tag)}`}
									className="text-muted-foreground text-sm font-normal underline-offset-2 hover:underline"
								>
									{tag}
								</Link>
							))}
